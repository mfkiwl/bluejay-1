...
always_ff @(posedge clk) begin
    ...
    ID__ir <= (rst | MEM__branch) ? NOP : ID__ready ? IF__ir : ID__ir;
end
...
always_ff @(posedge clk) begin
    ...
    EX__ctrl_flow <= (rst | ID__stall | MEM__branch) ? CTRL_FLOW__PC_PLUS_FOUR : EX__ready ? ID__ctrl_flow : EX__ctrl_flow;
...
always_ff @(posedge clk) begin
    ...
    MEM__ctrl_flow <= (rst | MEM__branch) ? CTRL_FLOW__PC_PLUS_FOUR : MEM__ready ? EX__ctrl_flow : MEM__ctrl_flow;
